...
class FilterMIDI:
    ...
    def __init__(
        ...
        folder_output,
        csv_file,
        logging: logging.Logger = logging.getLogger(name="FilterMIDI"),
        instruments=[0],
        write_mode="a",
        replication=False,
    ) -> None:
        ...
        if os.path.exists(self._csv_file):
            self._csv_file_data = pd.read_csv(
                self._csv_file, names=["previous_midi_file_name", "new_midi_file_name"]
            )
        else:
            # Otherwise create a new csv file
            self._csv_file_data = pd.DataFrame(
                columns=["previous_midi_file_name", "new_midi_file_name"]
            )
            self._csv_file_data.to_csv(
                self._csv_file, index=False, mode=self._write_mode
            )
    ...
    def check_replication_midi(self, midi_file: str) -> bool:
        ...
        if not self._replication:
            if (
                len(midi_file) == 0
                or not midi_file
                in self._csv_file_data["previous_midi_file_name"].values
            ):
                return True
        else:
            return True
    ...
    def has_instruments(self, midi_file: str) -> bool:
        ...
        return all(
            instrument.program in self._instruments for instrument in midi.instruments
        )
    ...
    def filter_and_extract_folder(self, midi_folder) -> None:
        ...
        self._logger.info(
            "Processing folder: {} with {} midi files".format(
                midi_folder, len(midi_files)
            )
        )
        ...
        with open(self._csv_file, self._write_mode) as csv_file:
            ...
            for midi_file in tqdm(midi_files):
                ...
                if self.check_replication_midi(midi_file) and self.has_instruments(
                    midi_file
                ):
